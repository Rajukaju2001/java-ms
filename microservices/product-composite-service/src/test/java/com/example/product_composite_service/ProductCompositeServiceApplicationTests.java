...
class ProductCompositeServiceApplicationTests {
  ...
  void setUp() {
    ...
    when(compositeIntegration.getRecommendations(anyInt()))
        .thenReturn(Flux.empty());
    when(compositeIntegration.getReviews(anyInt()))
        .thenReturn(Flux.empty());
    ...
    // ── create/delete mocks — these were missing ────────────────────

    when(compositeIntegration.createProduct(any()))
        .thenReturn(Mono.just(new Product(1, "name", 1, "mock-address")));
    when(compositeIntegration.createRecommendation(any()))
        .thenReturn(Mono.just(new Recommendation(1, 1, "a", 1, "c", "mock-address")));
    when(compositeIntegration.createReview(any()))
        .thenReturn(Mono.just(new Review(1, 1, "a", "s", "c", "mock-address")));
