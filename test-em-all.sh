...
function testCompositeCreated() {
    ...
    PROD_ID=$(echo $RESPONSE | jq .productId)
    REC_LEN=$(echo $RESPONSE | jq ".recommendations | length")
    REV_LEN=$(echo $RESPONSE | jq ".reviews | length")

    echo "  productId=$PROD_ID, recommendations=$REC_LEN, reviews=$REV_LEN"

    if [ "$PROD_ID" != "$PROD_ID_REVS_RECS" ] || [ "$REC_LEN" != "3" ] || [ "$REV_LEN" != "3" ]; then
        return 1
    fi
